-- Complete SQL for Real-time Messaging System (Fixed)
-- Copy ALL of this and run in your Supabase SQL Editor

-- ============================================
-- STEP 1: Enable Realtime on Tables (Safe Version)
-- ============================================
-- Only add tables if they're not already in the publication

-- Check and add messages table
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Check and add conversations table
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
...
-- 4. Send a message and watch it appear instantly at the BOTTOM! 💬
-- 5. The other user will see it in real-time too
-- 6. New messages always appear at the bottom (newest messages at bottom)
